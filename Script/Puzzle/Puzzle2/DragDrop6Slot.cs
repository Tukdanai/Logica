...
public class DragDrop6Slot : MonoBehaviour, IPointerDownHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    [SerializeField] Puzzle2Box puzzleBox6;
    ...
    public void OnPointerDown(PointerEventData eventData)
    {
        ...
        puzzleBox4.isDropped = false;
        puzzleBox5.isDropped = false;
        puzzleBox6.isDropped = false;
    }
    ...
    public void CheckBoxToAdd()
    {
        ...
        else if(puzzleBox4.isDropped == true && puzzleBox4.itemInBox.Count == 0)
        {
            PuzzleBoxFill.Play();
            puzzleBox4.itemInBox.Add(item);
            puzzleBox4.boxSlot[0].Item = item;
            puzzleBox4.boxSlot[0].image.GetComponent<Image>().color = new Color(1f, 1f, 1f, 1f);
            if(item != null && item.ItemName != "Close_Bracket" && item.ItemName != "Open_Bracket")
            {
                puzzleInventory.RemovePuzzleInventoryItem(item);
            }
        }
        else if(puzzleBox5.isDropped == true && puzzleBox5.itemInBox.Count == 0)
        {
            PuzzleBoxFill.Play();
            puzzleBox5.itemInBox.Add(item);
            puzzleBox5.boxSlot[0].Item = item;
            puzzleBox5.boxSlot[0].image.GetComponent<Image>().color = new Color(1f, 1f, 1f, 1f);
            if(item != null && item.ItemName != "Close_Bracket" && item.ItemName != "Open_Bracket")
            {
                puzzleInventory.RemovePuzzleInventoryItem(item);
            }
        }
        else if(puzzleBox6.isDropped == true && puzzleBox6.itemInBox.Count == 0)
        {
            PuzzleBoxFill.Play();
            puzzleBox6.itemInBox.Add(item);
            puzzleBox6.boxSlot[0].Item = item;
            puzzleBox6.boxSlot[0].image.GetComponent<Image>().color = new Color(1f, 1f, 1f, 1f);
            if(item != null && item.ItemName != "Close_Bracket" && item.ItemName != "Open_Bracket")
            {
                puzzleInventory.RemovePuzzleInventoryItem(item);
            }
